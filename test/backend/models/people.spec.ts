...
describe("backend/models/people", function () {
  ...
  it("invalid nameInBanner seperated by too many spaces", async function () {
    ...
    return expect(people).to.be.eventually.rejectedWith(BadRequest, "expected nameInBanner to contain only two words, seperated by space");
  });
  ...
  it("invalid nameInBanner with first word more than one character", async function () {
    ...
    return expect(people).to.be.eventually.rejectedWith(BadRequest, "expected first word in nameInBanner to have a length of 1");
  });
});
