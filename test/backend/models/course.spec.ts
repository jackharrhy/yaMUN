...
describe.only("backend/models/course", function () {
  ...
  it("find courses by subject", async function () {
    const courses = await Course.search({subject: "ANTH", page: 0, limit: 20});
    courses.every(c => expect(c).property("subject").to.equal("ANTH"));
  });
  
  it("find courses with non-existant subject", async function () {
    const courses = await Course.search({subject: "MAGIC", page: 0, limit: 20});
  ...
  it("find courses taught by certain prof", async function () {
    const courses = await Course.search({prof: "M Bartha", page: 0, limit: 20});
    courses.every(c => {
      c.sections.every(s => {
  ...
  it("test pagination", async function () {
    ...
    for(let i = 0; i < coursesPageOne.length; i++) {
      expect((coursesPageOne[i].subject == coursesPageTwo[i].subject) && (coursesPageOne[i].number == coursesPageTwo[i].number)).to.equal(false);
    }
  });
});
