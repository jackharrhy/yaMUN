...
describe("backend/models/bookmark", function () {
  ...
  it("create user bookmarks, add two courses to it", async function () {
    const bookmark = await Bookmark.findOrCreateByUserId(userId);
    await bookmark.addCourse(testSemester1Sids[0]);
    await bookmark.addCourse(testSemester1Sids[1]);
    expect(bookmark.courses).to.contain(testSemester1Sids[0]);
    expect(bookmark.courses).to.contain(testSemester1Sids[1]);
  });
  ...
  it("create user bookmarks, add a course and then delete it", async function () {
    ...
    await bookmark.removeCourse(testSemester1Sids[0]);
    expect(bookmark.courses).not.to.contain(testSemester1Sids[0]);
    expect(bookmark.courses).to.be.length(0);
  });

  it("create user bookmarks, add a non-existant course sid", async function () {
    const bookmark = await Bookmark.findOrCreateByUserId(userId);
