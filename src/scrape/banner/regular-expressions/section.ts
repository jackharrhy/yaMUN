...
import type { ISection, ISemester } from "../../../types";
...
export const SECTION_REGEX =
  /^.{37} (?<section>(\d|[A-Z]){3}) (?<crn>\d{5}) .{38}((?<scheduleType>([A-Z]|&){3})|   )   .{11} (?<phoneOne>Y|N|D|A) (?<phoneTwo>Y|N|D|A)  (?<waitList>Y|N)   (?<preCheck>Y|N)    ((?<reserved>Y|N|D)| )   (?<attr>(L|Q|W|G|R| ){5}) (?<creditHours>( |\d){3})  (?<billedHours>(( |\d){3})| NA)  ((Primary - (?<primaryInstructor>[A-Z] (\w| ){11}) ((?<secondaryInstructor>[A-Z] (\w| ){11})|.*))|.*)/;
...
export function matchToSection(
  ...
  const sid = createSemesterId(semester, crn);

  return {
    ...sectionMatch,
    sid,
    primaryInstructor: null,
    // TODO primaryInstructor: sectionMatch.primaryInstructor?.trim() ?? null,
    secondaryInstructor: null,
    // TODO secondaryInstructor: sectionMatch.secondaryInstructor?.trim() ?? null,
    crn,
